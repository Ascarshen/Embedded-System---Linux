...
//--------------------------AP加------------------------------------
//#include "include/exp_ops.h"
#include "src/include/serial.h"
#include "src/include/msg_queue.h"
#include "src/include/network_card.h"
#include "src/include/toollib.h"
//------------------------------------------------------------------
...
//bool HWNTconnected = false;
//bool ZTNTconnected = false;
int ZXconnectfd = -1;
int HWconnectfd = -1;
//int ZXNTconnectfd = -1;
//int HWNTconnectfd = -1;
...
int DCR = -9999;	//读配置文件
int UCR = -9999;	//读配置文件
int OUT_Count = 5;  //读配置文件,hfp
int IN_Count = 5;     //读配置文件,hfp
...
static char HWNTsocket_recvbuf[2048];
static char ZXNTsocket_recvbuf[2048];
...
static int network_real_info(struct _msgbuf * message)
{
	    int msgid = 0;
		memset(message, 0, sizeof(struct _msgbuf));
		if(msgrcv(msgid, message, sizeof(struct _msgbuf) - sizeof(long), SIGDAQ, IPC_NOWAIT)<0)
		{
			if(errno==ENOMSG)
			return -1;
		}
#ifdef DEBUG__MSG_QUEUE		
		switch (message->mtype)
		{
			case SIGDAQ://获取相应的消息(信号)
				printf("ip:%s si:%0.2f sq:%0.2f\n",message->ip,message->si,message->sq);
				break;
			default:
			    break;
		}
...
//------------------------------------------------------------------
        ...
        //std::cout<<"\r\nHWconnected:"<<HWconnected<<std::endl;
        while (HWconnected)
        {
...
//------------------------------------------------------------------
        ...
        //std::cout<<"\r\nZXconnected:"<<ZXconnected<<std::endl;
        while (ZXconnected)
        {
            sleep(sleepTime);
            char socket_heartpadload[50];
...
//        strcat(OTHERpayload, TEM.c_str()); //拼接TEM
        ...
        sprintf(qiehuan, "%05d", OUT_Count); //
        strcat(OTHERpayload, qiehuan);
        strcat(OTHERpayload, ",UCN");
        sprintf(qiehuan, "%05d", IN_Count ); //
        strcat(OTHERpayload, qiehuan);

        char Rsi[6];
        strcat(OTHERpayload, ",RSI-"); //RSI-85
        sprintf(Rsi, "%0.1f", rssi);    //
        strcat(OTHERpayload, qiehuan);
...
void *HWnTrip(void *ptr)
{
    ...
    std::cout<<HWconnectfd<<std::endl;
    while (true)
    {
        //差分服务器连接socket
        HWNTcilentfd = socket(AF_INET, SOCK_STREAM, 0);
        //socket连接返回值为-1，则联网失败
        if (HWNTcilentfd == -1)
        {
            //printf("HW_NTcilentfd error!\n");
        }
        //创建TCP连接
        while (HWNTconnectfd == -1) 
        {
            bzero(&NT_s_add, sizeof(struct sockaddr_in));
            
            NT_s_add.sin_family = AF_INET;
            //inet_addr为数据转换
            NT_s_add.sin_addr.s_addr = inet_addr(NTipaddr.c_str()); 
            NT_s_add.sin_port = htons(atoi(NTportnum.c_str()));
            //建立一个与端口的连接
            HWNTconnectfd = connect(HWNTcilentfd, (struct sockaddr *)(&NT_s_add), sizeof(struct sockaddr)); 
            // usleep(100000);
            //打印连接返回值
            //printf("HW_NTconnectfd %d , connect .. !\n", HWNTconnectfd);
            //返回值不是-1表示连接已建立，给出差分服务连接建立提示信息
            if (HWNTconnectfd != -1)
            {
                std::cout << "\r\n\r\nHW_NTrip connected.\r\n\r\n" << std::endl;
                HWNTconnected = true;
        ...
        while (HWNTconnected)
        {
            //std::cout << "HWNTconnected GET 0" << std::endl;
            NTname = config[5];
            ...
            send(HWNTcilentfd, NTsocket_sendbuf.c_str(), NTsocket_sendbuf.length(), 0);
            bzero(HWNTsocket_recvbuf, 2048);
            recbyte = read(HWNTcilentfd, HWNTsocket_recvbuf, 2048);
            // if (NTsocket_recvbuf[0] == '2' && NTsocket_recvbuf[1] == '0')
            //     break;
            if (recbyte > 0)
            {
                //std::cout << "\r\n HW_NT recbyte: \r\n" << recbyte << std::endl;
                //std::cout << "\r\n HW_NTsocket_recvbuf: \r\n" << HWNTsocket_recvbuf << std::endl;
                HW_NTgps = true;
                break;
            }
            usleep(300000);
        ...
        //while (HWNTconnected)
        //while (ZX_NTgps==false && HW_NTgps==true)
        while (HW_NTgps==true)  
        {
            sleep(1);
            /*gga*/
            ...
            {
                send(HWNTcilentfd, Serial_Mcu.GGA, Serial_Mcu.GGA_DatLen, 0);
                //std::cout << "HW NTrip GGA data has send: " << Serial_Mcu.GGA << std::endl;
            ...
            if (true)
            {
                ...
                bzero(HWNTsocket_recvbuf, 2048);
                recbyte = read(HWNTcilentfd, HWNTsocket_recvbuf, 2048);
                //std::cout<<"\r\nHWNTsocket_recvbuf recbyte:\r\n"<< recbyte <<std::endl;
                if (recbyte > 0)
                {
                    char startNT[2] = {'N', 'T'};
                    char end[2] = {0x0d,0x0a};
                    //char end = 0x0d;
                    Serial_Mcu.WritePort(startNT, 2);
                    Serial_Mcu.WritePort(HWNTsocket_recvbuf, recbyte);
                    Serial_Mcu.WritePort(end, 2);
...
void *ZXnTrip(void *ptr)
{
    ...
    {
        //创建差分服务器连接socket
        ZXNTcilentfd = socket(AF_INET, SOCK_STREAM, 0);
        //socket连接返回值为-1，则联网失败
        if (ZXNTcilentfd == -1)
        {
            //printf("ZX_NTrip socket create error!\n");
            close(ZXNTcilentfd);
        }
        
        
        //创建TCP连接
        while (ZXNTconnectfd == -1)
        {
            bzero(&NT_s_add, sizeof(struct sockaddr_in));
            NT_s_add.sin_family = AF_INET;
            //IP地址，inet_addr为数据转换
            ...
            //建立一个与端口的连接
            ZXNTconnectfd = connect(ZXNTcilentfd, (struct sockaddr *)(&NT_s_add), sizeof(struct sockaddr)); 
            // usleep(100000);
            //打印连接返回值
            ...
            else if (NTconnectCnt++ > 3)
            {
                ...
                NTconnectCnt = 0;
                //差分服务器连接失败提示信息
                //std::cout << "ZX_NTrip connect error！reconnecting ......" << std::endl;
                break;
            }
            //延时300ms
            usleep(300000);
        ...
        { 
            ...
            send(ZXNTcilentfd, NTsocket_sendbuf.c_str(), NTsocket_sendbuf.length(), 0);
            //接收差分服务器返回的数据，并获取差分数据的字节数
            bzero(ZXNTsocket_recvbuf, 2048);
            recbyte = read(ZXNTcilentfd, ZXNTsocket_recvbuf, 2048);
            // if (NTsocket_recvbuf[0] == '2' && NTsocket_recvbuf[1] == '0')
            //     break;
            //数据长度大于0，表明收到差分服务器返回数据，连接成功
            if (recbyte > 0)
            {
                //打印差分服务器返回数据字节数
                //std::cout << "\r\nZX_NTrip recbyte: " << recbyte << std::endl;
                //打印差分服务器返回数据
                //std::cout << "\r\nZX_NTrip socket_recvbuf: " << ZXNTsocket_recvbuf << std::endl;
                //中兴模块连接差分服务器成功标识
                ZX_NTgps = true;
                break;
            }
        ...
        //ZX_NTgps
        //while (ZTNTconnected)
        //中兴模块连接差分服务器成功，接下来获取将差分数据并转发给STM32   
        //while (ZX_NTgps==true)
        while(0)
        {
            //延时1s
            sleep(1);
            //接收到差分数据后，将差分数据转发给32，给出过程提示信息
            //std::cout << "ZXNTrip data has received. Next,send to STM32......" << std::endl;
            
            //向服务器发送获取到的GGA数据
            if (Serial_Mcu.GGA_Send)
            {
                //向服务器发送GGA数据
                send(ZXNTcilentfd, Serial_Mcu.GGA, Serial_Mcu.GGA_DatLen, 0);
                //打印发送给差分服务器的GGA数据
                //std::cout << "\r\nZX NTrip GGA data has send : " << Serial_Mcu.GGA << std::endl;
            }
            
            //接收差分服务器下发的数据
            //if (true)   //whichclient ==1
            if(false)
            {
                ...
                if (recbyte > 0)
                {
                    ...
                    //向STM32发送帧头
                    Serial_Mcu.WritePort(start, 2);
                    //向STM32发送差分数据
                    Serial_Mcu.WritePort(ZXNTsocket_recvbuf, recbyte);
                    //向STM32发送帧尾
                    Serial_Mcu.WritePort(end, 2);
                }
                //没有收到差分数据，给出报错提示信息
...
//------------------------------------------------------------------
/*
add 2018年9月17日16:04:00
BY online

*/
//
void * pthread_deal_queue(void *)
{
	if (init_net_card()<0)
	{
		printf("err of init card");
        exit(-1);
	}
...
*/        
       ...
       // sleep(120);
        ...
        if (true)
        {
            system("./test_ap 192.168.5.143 9090");
...
*/
        ...
        //-----------------华为和中兴上传通道切换---------------------
        if (HWconnected && ZXconnected)
        {
						//默认使用中兴专网模块进行数据上传
			//华为->中兴 切入 		
            if (ZXcilentfd > 0 && Cin > UCR && rssi > URS)
            {
				static int in_counter=0;
				if(in_counter++<OUT_Count)
					continue;
				in_counter=0;
                //上传数据句柄为中兴联网句柄
                cilentfd = ZXcilentfd;
			...
			//中兴->华为 切出
            else if (HWcilentfd > 0 && Cin < DCR && rssi < DRS)
            {
				static int out_counter=0;
				if(out_counter++<IN_Count )
					continue;
				out_counter=0;				
                //上传数据句柄为中兴联网句柄
                cilentfd = HWcilentfd;
