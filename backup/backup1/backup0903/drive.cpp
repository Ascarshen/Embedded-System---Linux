...
//"您已越界"
char YUEJIE[] = {0xC4, 0xFA, 0xD2, 0xD1, 0xD4, 0xBD, 0xBD, 0xE7};       
//"您已进入违规区域"                                         
char JINRU[] = {0xC4, 0xFA, 0xD2, 0xD1, 0xBD, 0xF8, 0xC8, 0xEB, 0xCE, 0xA5, 0xB9, 0xE6, 0xC7, 0xF8, 0xD3, 0xF2}; 
//前方有停止线，请减速
char STOP[] = {0xFD,0x00,0x0E,0x01,0x01,0xC7,0xB0,0xB7,0xBD,0xD3,0xD0,0xCD,0xA3,0xD6,0xB9,0xCF,0xDF};
//违规进入电子围栏区域
char WEILAN[] = {0xCE, 0xA5, 0xB9, 0xE6, 0xBD, 0xF8, 0xC8, 0xEB, 0xB5, 0xE7, 0xD7, 0xD3, 0xCE, 0xA7, 0xC0, 0xB8, 0xC7, 0xF8, 0xD3, 0xF2};

//"车辆预热"           
char YURE[] = {0xB3, 0xB5, 0xC1, 0xBE, 0xD4, 0xA4, 0xC8, 0xC8};  
//"您已疲劳驾驶"                               
char PILAO[] = {0xC4, 0xFA, 0xD2, 0xD1, 0xC6, 0xA3, 0xC0, 0xCD, 0xBC, 0xDD, 0xCA, 0xBB};    
//"超时停车"    
char CHAOSHI[] = {0xB3, 0xAC, 0xCA, 0xB1, 0xCD, 0xA3, 0xB3, 0xB5};                              
...
//            sleep(1);
          ...
          //上传速度信息
          if(true){
          	char socket_speed[50];
            //char Crc1[2];
            //请求配置次数计数值清零
            bzero(socketCnt, 5);
            //拼接计数值
            sprintf(socketCnt, "%05d", socket_cnt);            
            strcpy(socket_speed, "##00");
            strcat(socket_speed, socketCnt);
            //拼接SSN字段
            strcat(socket_speed, ",GGDC,SSN");
            strcat(socket_speed, SSN.c_str());
            //拼接TYPGGDC
            strcat(socket_speed, ",TYPGGVA");
            //拼接VNB字段
            strcat(socket_speed, ",VNB");
            strcat(socket_speed, VNB.c_str()); 
						//拼接ENB字段
            strcat(socket_speed, ",ENB");
            strcat(socket_speed, ENB.c_str()); 
            //拼接LGT字段
            strcat(socket_speed, ",LGT");
            //strcat(socket_speed, LGT.c_str()); 
            //拼接LTT字段
            strcat(socket_speed, ",LTT");
            //strcat(socket_speed, LTT.c_str()); 
            //拼接SPD字段
            strcat(socket_speed, ",SPD");
            //strcat(socket_speed, SPD.c_str()); 
            //拼接MSP字段
            strcat(socket_speed, ",MSP");
            //strcat(socket_speed, MSP.c_str()); 
             //拼接SDT字段
            strcat(socket_speed, ",SDT");
            strcat(socket_speed, BDdate); 
            //拼接DRT字段
            strcat(socket_speed, ",DRT");
            strcat(socket_speed, BDdate);            
            //拼接逗号分隔符       
            strcat(socket_speed, ",");
						//拼接校验位
            char socket_speed_CRC = '#'; //CRC
            for (uint16_t k = 1; k < strlen(socket_speed); k++)
            {
                socket_speed_CRC ^= socket_speed[k];
            }
            sprintf(Crc, "%02X", socket_speed_CRC);
            strcat(socket_speed, Crc); //CRC
            strcat(socket_speed, "@@");

            //清空配置信息存储数组
            bzero(socket_recvbuf, 1024);
            //发送请求服务器配置数据包
            send(HWcilentfd, socket_speed, strlen(socket_speed), 0);
            socket_cnt++;
            //打印请求服务器配置数据包
            printf("HW speed sended\n");
            printf("HW speed sended:%s\n", socket_speed);
            //读取服务器下发的配置数据
            recbyte = read(HWcilentfd, socket_recvbuf, 512);
           	//无数据，读取失败，给出报错信息
            if (recbyte == -1)
            {
                printf("read service data fail !\n");
            ...
            { 
                printf("HW speed information received\n");
                std::cout << "HW speed recvbuf: " << socket_recvbuf << std::endl;
            } 
            //超时判断，超时给出提示信息
            else
            {
...
void *BDsend(void *ptr)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < Maxlength; i++)
        {
            ...
            if(speednew>=speedlimit && speednew<=speedmax){
                //chaosu
